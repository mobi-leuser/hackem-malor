...
static boolean NDECL(bodyshaping);
static boolean FDECL(absorbmaterials, (int));
static boolean FDECL(applymaterials, (int));
...
STATIC_OVL NEARDATA const char *tech_names[] = {
	...
	"absorb armor", /* 58 */
	"absorb weapon", /* 59 */
	"apply body hardness", /* 60 */
	"apply body swiftness", /* 61 */
	"apply body energy", /* 62 */
	"apply body movement", /* 63 */
	"",                 /* 64 */
	""
};
 ...
 * Are you sure you don't want to use u.tech_inuse[techid(tech_no)],  which is the
...
int tech;
{
	...
	if (u.tech_inuse[techid(i)]) {
		int tinuse = u.tech_inuse[techid(i)];
...
int *tech_no;
{
	...
	for (i = 0; i < MAXTECH; i++) {
		...
		} else if (techid(i) == T_BODY_SHAPING) {
			if (!u.bodyshape) {
				u.bodyshape = BODY_HUMAN;
			}
			char curshape[40];
			if (u.bodyshape == BODY_HUMAN) {
				Sprintf(curshape, "Humanoid shape");
			} else if (u.bodyshape == BODY_TRAVEL) {
				Sprintf(curshape, "Travel form");
			} else if (u.bodyshape == BODY_SHIELD) {
				Sprintf(curshape, "Shield form");
			} else if (u.bodyshape == BODY_BLADE) {
				Sprintf(curshape, "Blade form");
			} else if (u.bodyshape == BODY_DIG) {
				Sprintf(curshape, "Drilling form");
			} else if (u.bodyshape == BODY_REST) {
				Sprintf(curshape, "Healing form");
			} else if (u.bodyshape == BODY_CONSERVE) {
				Sprintf(curshape, "Conservation form");
			} else if (u.bodyshape == BODY_HIDE) {
				Sprintf(curshape, "Camouflage form");
			}
			Sprintf(prepare, "body shaping (%s)", curshape);
		} else if (techid(i) == T_APPLY_HARD) {
			if (u.bodyhard > 0) {
				Sprintf(prepare, "Apply body hardness (%d)", u.bodyhard);
			} else {
				continue;
			}
		} else if (techid(i) == T_APPLY_SWIFT) {
			if (u.bodyswift > 0) {
				Sprintf(prepare, "Apply body swiftness (%d)", u.bodyswift);
			} else {
				continue;
			}
		} else if (techid(i) == T_APPLY_ENERGY) {
			if (u.bodyenergy > 0) {
				Sprintf(prepare, "Apply body energy (%d)", u.bodyenergy);
			} else {
				continue;
			}
		} else if (techid(i) == T_APPLY_MOVE) {
			if (u.bodymove > 0) {
				Sprintf(prepare, "Apply body movement (%d)", u.bodymove);
			} else {
				continue;
			}
		} else {
...
bodyshape(VOID_ARGS)
{
	if (u.bodyshapehold < 1000) {
		u.bodyshapehold += 1000;
	} else {
		if (u.bodyshape == BODY_HUMAN) { /* should not have to modify anything */
		} else if (u.bodyshape == BODY_TRAVEL) {
			u.uhitinc += 10;
			u.udaminc += 10;
		} else if (u.bodyshape == BODY_SHIELD) {
			...
			u.uhitinc += 1;
			u.udaminc += 1;
			u.uprops[SLOW_DIGESTION].intrinsic = FALSE;
		} else if (u.bodyshape == BODY_HIDE) {
			u.uhitinc += 2;
			u.udaminc += 2;
			u.uprops[STEALTH].intrinsic = FALSE;
		}
		u.bodyshape = u.bodyshapehold - 1000;
		if (u.bodyshape == BODY_HUMAN) { /* should not have to modify anything */
		} else if (u.bodyshape == BODY_TRAVEL) {
			u.uhitinc -= 10;
			u.udaminc -= 10;
			...
			u.uhitinc -= 1;
			u.udaminc -= 1;
			u.uprops[SLOW_DIGESTION].intrinsic = TRUE;
		} else if (u.bodyshape == BODY_HIDE) {
			u.uhitinc -= 2;
			u.udaminc -= 2;
			u.uprops[STEALTH].intrinsic = TRUE;
		}
		pline("Your reshaping is done");
		return 0;
	}
	return 1; /* done */
}
...
int tech_no;
{
	...
	/* check timeout */
	if (u.tech_inuse[techid(tech_no)]) {
	...
	switch (techid(tech_no)) {
		...
		case T_BODY_SHAPING:
			...
			if (doneres) {
				res = 1;
				set_occupation(bodyshape, "adjusting your body shape", 0);
			}
			t_timeout = 0;
			break;
		case T_ABSORB_ARMOR:
		case T_ABSORB_WEAPON:
			if (!u.bodyshape || u.bodyshape == BODY_HUMAN) {
				boolean stayin = TRUE;
				while (stayin) {
					stayin = FALSE;
					doneres = FALSE;
					doneres = absorbmaterials(techid(tech_no));
					if (doneres) {
						stayin = TRUE;
					}
				}
			} else {
				You("may only use this technique while in humanoid shape");
			}
			t_timeout = 0;
			break;
		case T_APPLY_HARD:
			...
			doneres = applymaterials(techid(tech_no));
			if (doneres) {
				res = 1;
			}
			t_timeout = 0;
		...
		case T_POWER_SHIELD:
			...
			break;
		case T_TELEKINESIS:
			/* Set u.tech_inuse[techid(i)]=1 temporarily so the use of telekinesis can be seen in shk.c */
			u.tech_inuse[techid(tech_no)] = 1;
			res = tech_telekinesis();
			u.tech_inuse[techid(tech_no)] = 0;
			if (res)
				t_timeout = rn1(100, 100);
			break;
		case T_FORCE_PUSH:
...
int dam;
{
	...
	if (!u.tech_inuse[T_POWER_SHIELD])
		return 0;
	...
	if (u.uen <= 0) {
		u.uen = 0;
		You("can no longer power shield with %s.", uarms ? yname(uarms) : "a shield");
		u.tech_inuse[techid(tech_no)] = 0;
	}
	context.botl = 1;
	return res;
...
struct attack *mattk;
{
	if (!u.tech_inuse[T_POWER_SHIELD] || !canspotmon(mtmp) 
...
tech_timeout()
{
	...
	if (u.tempmoveuse > 0) {
		if (u.tempmove == BODY_MOVE_FLY) {
			if (u.tempmoveuse < 3) {
				You("are nearly done flying.");
			}
			if (!rn2(10)) {
				u.tempmoveuse--;
			}
		...
		} else if (u.tempmove == BODY_MOVE_WATER_WALK) {
			if (u.tempmoveuse < 3) {
				You("are nearly done walking on water.");
			}
			if (!rn2(10)) {
				u.tempmoveuse--;
			}
			if (u.tempmoveuse <= 0) {
				u.tempmove = 0;
				You("are no longer walking on water.");
			}
		}
	} else if (u.tempmove > 0) {
		u.tempmove = 0;
	}
...
docalm()
{
	...
	for (i = 0; i < MAXTECH; i++) {
		tech = techid(i);
		if (tech != NO_TECH && u.tech_inuse[techid(i)]) {
			 aborttech(tech);
			 n++;
		}
	}
...
specialtychoice()
{
	...
	} else if (!strcmpi(buf, "broad sword") || !strcmpi(buf, "broadsword")) {
...
bodyshaping()
{
	...
	if (u.bodyshape == BODY_HUMAN) {
		pline("Humanoid form is your current form");
	} else {
		pline("'Human' is humanoid form");
	}
	if (u.bodyshape == BODY_TRAVEL) {
		pline("Travel form is your current form");
	} else {
		pline("'Travel' is a fast travel form");
	}
	if (u.bodyshape == BODY_SHIELD) {
		pline("Defensive shield form is your current form");
	} else {
		pline("'Shield' is a defensive focused form");
	}
	if (u.ulevel >= 5) {
		if (u.bodyshape == BODY_BLADE) {
			pline("Offensive blade form is your current form");
		} else {
			pline("'Blade' is an offensive focused form");
		}
	}
	if (u.bodyshape == BODY_DIG) {
		pline("Digging form is your current form");
	} else {
		pline("'Dig' is a digging form");
	}
	if (u.bodyshape == BODY_REST) {
		pline("Healing form is your current form");
	} else {
		pline("'Rest' is a healing increasing");
	}
	if (u.bodyshape == BODY_CONSERVE) {
		pline("Conservation form is your current form");
	} else {
		pline("'Conserve' is slow with reduce food required");
	}
	if (!u.uprops[STEALTH].intrinsic) { /* if they already have intrinsic stealth, don't allow the "hide" form */
		if (u.bodyshape == BODY_HIDE) {
			pline("Hide form is your current form");
		} else {
			pline("'Hide' attempts to match your surroundings");
		}
	}
	Strcpy(promptbuf, "Enter the name of the form you want to take");
	getlin(promptbuf, buf);
	if (u.bodyshape != BODY_HUMAN && (!strcmpi(buf, "human") || !strcmpi(buf, "normal"))) {
		u.bodyshapehold = BODY_HUMAN;
	} else if (u.bodyshape != BODY_TRAVEL && (!strcmpi(buf, "travel") || !strcmpi(buf, "horse"))) {
		u.bodyshapehold = BODY_TRAVEL;
	} else if (u.bodyshape != BODY_SHIELD && !strcmpi(buf, "shield")) {
		u.bodyshapehold = BODY_SHIELD;
	} else if (u.bodyshape != BODY_BLADE && !strcmpi(buf, "blade") && u.ulevel >= 5) {
		u.bodyshapehold = BODY_BLADE;
	} else if (u.bodyshape != BODY_DIG && !strcmpi(buf, "dig")) {
		u.bodyshapehold = BODY_DIG;
	} else if (u.bodyshape != BODY_REST && !strcmpi(buf, "rest")) {
		u.bodyshapehold = BODY_REST;
	} else if (u.bodyshape != BODY_CONSERVE && !strcmpi(buf, "conserve")) {
		u.bodyshapehold = BODY_CONSERVE;
	} else if (u.bodyshape != BODY_HIDE && !strcmpi(buf, "hide") && !u.uprops[STEALTH].intrinsic) {
		u.bodyshapehold = BODY_HIDE;
	} else {
		return FALSE;
	}
	return TRUE;
...
int tech;
{
	...
	if (tech == T_ABSORB_ARMOR) {
		...
		otmp = getobj(weapon_types, "absorb");
	}
	if (otmp) {
		sprintf(promptbuf, "Do you want to absorb %s (yes/no)", doname(otmp));
		getlin(promptbuf, buf);
		if (!strcmpi(buf, "yes")) {
			if (otmp->cursed || otmp->oprops & ITEM_TOUGH || is_ammo(otmp) || otmp->oartifact) {
				pline("You are not able to absorb this item for some reason");
			} else {
				You("aborb the item into the essence of your being!");
				if (otmp->oprops & ITEM_FIRE) { /*These properties are separate so multi-energy items count multiple times. */
					u.bodyenergy++;
				}
				if (otmp->oprops & ITEM_FROST) {
					u.bodyenergy++;
				}
				if (otmp->oprops & ITEM_SHOCK) {
					u.bodyenergy++;
				}
				if (otmp->oprops & ITEM_VENOM) {
					u.bodyenergy++;
				}
				if (otmp->oprops & ITEM_SIZZLE) {
					u.bodyenergy++;
				}
				if (otmp->oprops & ITEM_SCREAM) {
					u.bodyenergy++;
				}
				if (otmp->oprops & ITEM_DECAY) {
					u.bodyenergy++;
				}
				if (otmp->oprops & ITEM_FLEX) {
					u.bodyswift++;
					u.bodymove++;
				}
				if (otmp->oprops & ITEM_OILSKIN) {
					u.bodymove++;
				}
				if (otmp->oprops & ITEM_PROWESS) {
					u.bodyswift++;
					u.bodymove++;
				}
				if (otmp->oprops & ITEM_SURF) {
					u.bodymove++;
				}
				if (otmp->oprops & ITEM_SWIM) {
					u.bodymove++;
				}
				if (otmp->oprops & ITEM_TELE) {
					u.bodymove++;
				}
...
int tech;
{
	...
	char promptbuf[BUFSZ];
	int maxapply, val1, val2;
	if (tech == T_APPLY_HARD && u.bodyhard > 0) {
		pline("Enter 'DR' or 'Damage Reduction' for damage reduction");
		pline("Enter 'AC' or 'Armor Class' for better armor class");
		sprintf(promptbuf, "How would you like to apply your body hardness?");
		getlin(promptbuf, buf);
		if (!strcmpi(buf, "dr") || !strcmpi(buf, "damage reduction") || !strcmpi(buf, "ac") || !strcmpi(buf, "armor class")) {
			maxapply = u.bodyhard;
			if (maxapply > 50) {
				maxapply = 50;
			}
			sprintf(promptbuf, "What level of power would you like to apply to your %s?  (Up to %d)", buf, maxapply);
			getlin(promptbuf, buf2);
			val1 = atoi(buf2);
			if (val1 > 0 && val1 <= maxapply) {
				maxapply = (int) (u.bodyhard / val1);
				sprintf(promptbuf, "How long would you like to apply your %d bonus to your %s (Up to %d)", val1, buf, maxapply);
				getlin(promptbuf, buf2);
				val2 = atoi(buf2);
				if (val2 > 0 && val2 <= maxapply) {
					u.bodyhard -= (val1 * val2);
					if (!strcmpi(buf, "dr") || !strcmpi(buf, "damage reduction")) {
						u.tempdr = val1;
						u.tempdruse = val2;
					} else if (!strcmpi(buf, "ac") || !strcmpi(buf, "armor class")) {
						u.tempac = val1;
						u.tempacuse = val1;
					}
				}
	...
	} else if (tech == T_APPLY_SWIFT && u.bodyswift > 0) {
		maxapply = u.bodyswift;
		if (maxapply > 50) {
			maxapply = 50;
		}
		sprintf(promptbuf, "How much speed would you like to apply? (Up to %d)", maxapply);
		getlin(promptbuf, buf);
		val1 = atoi(buf);
		if (val1 > 0 && val1 <= maxapply) {
			maxapply = (int) (u.bodyswift / val1);
			sprintf(promptbuf, "How long would you like to apply your %d speed? (Up to %d)", val1,  maxapply);
			getlin(promptbuf, buf2);
			val2 = atoi(buf2);
			if (val2 > 0 && val2 <= maxapply) {
				u.bodyswift -= (val1 * val2);
				u.tempspeed = val1;
				u.tempspeeduse = val2;
			}
		}
	} else if (tech == T_APPLY_ENERGY && u.bodyenergy > 0) {
		pline("Energy types (fire, cold, shock, poison, acid, sonic)");
		getlin("What energy type do you want to use?", buf);
		boolean cball = FALSE;
		if (!strcmpi(buf, "fire") || !strcmpi(buf, "f")) {
			u.tempenergytype = ITEM_FIRE;
			cball = TRUE;
		} else if (strcmpi(buf, "cold") || strcmpi(buf, "frost") || strcmpi(buf, "c")) {
			u.tempenergytype = ITEM_FROST;
			cball = TRUE;
		} else if (strcmpi(buf, "shock") || strcmpi(buf, "lightning")) {
			u.tempenergytype = ITEM_SHOCK;
			cball = TRUE;
		} else if (strcmpi(buf, "venom") || strcmpi(buf, "poison") || strcmpi(buf, "p")) {
			u.tempenergytype = ITEM_VENOM;
			cball = TRUE;
		} else if (strcmpi(buf, "sizzle") || strcmpi(buf, "acid") || strcmpi(buf, "a")) {
			u.tempenergytype = ITEM_SIZZLE;
			cball = TRUE;
		} else if (strcmpi(buf, "scream") || strcmpi(buf, "sonic")) {
			u.tempenergytype = ITEM_SCREAM;
			cball = TRUE;
		}
		if (cball) {
			maxapply = u.bodyenergy;
			if (maxapply > 50) {
				maxapply = 50;
			}
			sprintf(promptbuf, "How much %s energy would you like to apply? (Up to %d)", buf, maxapply);
			getlin(promptbuf, buf2);
			val1 = atoi(buf2);
			if (val1 > 0 && val1 <= maxapply) {
				maxapply = (int) (u.bodyenergy / val1);
				sprintf(promptbuf, "How long would you like to apply your %d %s energy? (Up to %d)", val1, buf, maxapply);
				getlin(promptbuf, buf2);
				val2 = atoi(buf2);
				if (val2 > 0 && val2 <= maxapply) {
					u.bodyenergy -= (val1 * val2);
					u.tempenergy = val1;
					u.tempenergyuse = val2;
				}
			}
		}
	} else if (tech == T_APPLY_MOVE && u.bodymove > 0) {
		if (u.bodymove >= 20) {
			pline("Movement types (phasing, flight, water walking)");
		} else if (u.bodymove >= 4) {
			pline("Movement types (flight, water walking)");
		} else {
			pline("Movement types (water walking)");
		}
		getlin("What movement type do you want to use?", buf);
		boolean cball = FALSE;
		if (!strcmpi(buf, "phasing") && u.bodymove >= 20) {
			incr_itimeout(&HPasses_walls, 3);
			pline("You phase for a short time!");
			u.bodymove -= 20;
			return TRUE;
		} else if ((!strcmpi(buf, "flight") || !strcmpi(buf, "fly")) && u.bodymove >= 4) {
			cball = TRUE;
		} else if (!strcmpi(buf, "water walking")) {
			cball = TRUE;
		}
		if (cball) {
			maxapply = u.bodymove;
			if (!strcmpi(buf, "flight") || !strcmpi(buf, "fly")) {
				maxapply = (int) (u.bodymove / 3);
			}
			if (maxapply >= 1) {
				if (maxapply > 50) {
					maxapply = 50;
				}
				sprintf(promptbuf, "How long would you like to %s? (Up to %d)", buf, maxapply);
				getlin(promptbuf, buf2);
				val2 = atoi(buf2);
				if (val2 > 0 && val2 <= maxapply) {
					if ((!strcmpi(buf, "flight") || !strcmpi(buf, "fly")) && u.bodymove >= 4) {
						u.bodymove -= (4 * val2);
						u.tempmove = BODY_MOVE_FLY;
						You("are now in flight.");
					} else if (!strcmpi(buf, "water walking")) {
						u.bodymove -= val2;
						u.tempmove = BODY_MOVE_WATER_WALK;
...
blitz_chi_strike()
{
	...
	u.tech_inuse[tech_no] = techlev(tech_no) + 4;
	return 1;
}
...
blitz_e_fist()
{
	...
	u.tech_inuse[tech_no] = rnd((int) (techlev(tech_no) / 3 + 1)) + d(1, 4) + 2;
	return 1;
}
...
tech_kiii()
{
	...
	You("scream \"KIIILLL!\"");
	aggravate();
	u.tech_inuse[tech_no] = rnd((int) (techlev(tech_no) / 6 + 1)) + 2;
	return 1;
}
...
tech_vanish()
{
	...
	u.tech_inuse[tech_no] = rn1(50, 50) + techlev(tech_no);

	if (!Invisible)
		 pline("In a puff of smoke,  you disappear!");
	if (!Fast)
		 You("feel more nimble!");
	incr_itimeout(&HInvis, u.tech_inuse[tech_no]);
	incr_itimeout(&HFast, u.tech_inuse[tech_no]);
...
tech_eviscerate()
{
	...
	aggravate();
	u.tech_inuse[tech_no] = d(2, 4) + techlev(tech_no) / 5 + 2;
	return 1;
}
...
tech_berserk()
{
	...
	u.tech_inuse[tech_no] = d(2, 8) + (techlev(tech_no) / 5) + 2;
	incr_itimeout(&HFast, u.tech_inuse[tech_no]);
	return 1;
}
...
tech_icearmor()
{
	...
	u.tech_inuse[tech_no] = d(multiplier, 6) + d(1, 8) + 2;
	return 1;
}
...
tech_primalroar()
{
	...
	aggravate();
	u.tech_inuse[tech_no] = d(4, 6) + (techlev(tech_no)) + 12;
	incr_itimeout(&HFast, u.tech_inuse[tech_no]);
...
tech_blink()
{
	...
	u.tech_inuse[tech_no] = rnd(techlev(tech_no) + 1) + 2;
	return 1;
}
...
tech_chiheal()
{
	...
	u.tech_inuse[tech_no] = techlev(tech_no) * 2 + 4;
	return 1;
}
...
tech_souleater()
{
	...
	u.tech_inuse[tech_no] = (techlev(tech_no) / 2) + weapon_modifier;
...
tech_shieldblock()
{
	...
	u.tech_inuse[tech_no] = (d(3, 4) + techlev(tech_no)) * 2;
	return 1;
}
...
tech_bloodmagic()
{
	...
	u.tech_inuse[tech_no] = (int) (techlev(tech_no) * 6 + 1) + 2;
	return 1;
}
